<x-admin-layout>
    ...
    <div class="px-4 md:px-4 py-12 relative z-10 ">

        {{-- Header Section --}}
        <div class="mb-10 text-center md:text-left">
            <h1 class="text-3xl font-bold text-slate-800 dark:text-white mb-2">Edit Data Kelas</h1>
            <p class="text-slate-500 dark:text-slate-400">
                Perbarui informasi untuk kelas: <span
                    class="font-bold text-slate-700 dark:text-slate-200">{{ $classroom->name }}</span>
            </p>
        </div>
        ...
        <div
            ...
            <div class="p-8 md:p-10">
                <form action="{{ route('admin.classrooms.update', $classroom) }}" method="POST" class="space-y-10">
                    @csrf
                    @method('PUT')
                                ...
                                <select name="name"
                                    ...
                                    @foreach (range(1, 6) as $i)
                                        <option value="Kelas {{ $i }}"
                                            {{ old('name', $classroom->name) == "Kelas $i" ? 'selected' : '' }}>
                                            Kelas {{ $i }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="relative group">
                                ...
                                <div class="relative">
                                    <input type="text" x-model="search"
                                        ...
                                        class="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition-all shadow-sm cursor-text placeholder-slate-400"
                                        {{-- Logic Placeholder: Jika ada ID terpilih, tampilkan Namanya --}}
                                    ...
                                    <div class="absolute right-4 top-1/2 -translate-y-1/2 flex items-center">
                                        <button type="button" x-show="selectedId" @click="clearSelection()"
                                            class="text-slate-400 hover:text-rose-500 mr-2 transition-colors">
                    ...
                    <div class="flex items-center justify-end gap-4">
                        ...
                        <button type="submit"
                            class="relative overflow-hidden px-8 py-3.5 bg-linear-to-r from-purple-700 to-blue-600 text-white font-semibold rounded-xl shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 hover:-translate-y-0.5 transition-all duration-200 group inline-flex items-center gap-2">
