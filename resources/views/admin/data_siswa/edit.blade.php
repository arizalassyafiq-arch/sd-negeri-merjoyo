<x-admin-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            <div>
                <h1 class="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Edit Data Siswa</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">
                    Memperbarui informasi untuk siswa: <span
                        class="font-bold text-slate-700 dark:text-slate-200">{{ $student->name }}</span>
                </p>
            </div>
        ...
        <form action="{{ route('admin.students.update', $student->id) }}" method="POST" class="space-y-8">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="status"
                                        class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-sm rounded-xl px-5 py-3.5 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition-all appearance-none shadow-sm cursor-pointer">
                                        <option value="active"
                                            {{ old('status', $student->status) == 'active' ? 'selected' : '' }}>Aktif
                                        </option>
                                        <option value="lulus"
                                            {{ old('status', $student->status) == 'lulus' ? 'selected' : '' }}>Lulus
                                        </option>
                                        <option value="pindah"
                                            {{ old('status', $student->status) == 'pindah' ? 'selected' : '' }}>Pindah
                                            Sekolah</option>
                                        <option value="drop_out"
                                            {{ old('status', $student->status) == 'drop_out' ? 'selected' : '' }}>Drop
                                            Out (DO)</option>
                                    </select>
                                    <span
                                        class="absolute right-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-500 pointer-events-none">expand_more</span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="address" rows="3"
                                    ...
                                    placeholder="Jalan, RT/RW, Kelurahan, Kecamatan">{{ old('address', $student->address) }}</textarea>
                                @error('address')
                                    <p class="text-rose-500 text-xs mt-1 font-semibold">{{ $message }}</p>
                                @enderror
                            </div>
            ...
            <div class="flex items-center justify-end pt-6">
                <button type="submit"
                    ...
                    <span class="mr-2">Update Data Siswa</span>
                    <span
